...
// mousedown : 박스의 위치와 마우스 위치의 차이를 계산 후 저장(마우스 움직임에 따라 도형도 똑같이 움직이기 위함)
//          indexMoving은 움직여야 하는 박스의 인덱스 저장
// mousemove : 움직일 박스가 있을 때 작동, 마우스 위치에 아까 계산한 차이를 빼고 이동(자연스러운 움직임)
// mouseup : 이 때부터 움직일 박스가 없으므로 indexMoving 변수 -1
let indexMoving = -1; // 어떤 박스가 움직일 지 확인
let boxs;
let mousePos;
...
window.onload = function () {
  ...
  for (let i = 0; i < boxs.length; i += 1) {
    const index = i;
    boxs[index].addEventListener('mousedown', (e) => {
      const left = boxs[index].offsetLeft;
      const top = boxs[index].offsetTop;
    ...
    boxs[index].addEventListener('mousemove', (e) => {
    ...
    boxs[index].addEventListener('mouseup', () => {
      indexMoving = -1;
    });
  }
};
